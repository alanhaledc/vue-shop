        ...
        <div class="item-list-wrap confirm-item-list-wrap">
          <div class="cart-item order-item">
            ...
            <ul class="cart-item-list">
              <li v-for="product in cartList" :key="product._id">
                <div class="cart-tab-1">
                  <div class="cart-item-pic">
                    <img :src="'static/images/'+product.productImage" alt="">
                  </div>
                ...
                <div class="cart-tab-2">
                  <div class="item-price">{{product.salePrice | formatPrice}}</div>
                </div>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{product.salePrice * product.productNum | formatPrice}}</div>
                </div>
              </li>
            </ul>
          </div>
          ...
          <div class="price-count">
            <ul>
              <li>
                ...
                <span>{{subPrice | formatPrice}}</span>
              </li>
              <li>
                <span>Shipping:</span>
                <span>{{shippingPrice | formatPrice}}</span>
              </li>
              <li>
                <span>Discount:</span>
                <span>{{discountPrice | formatPrice}}</span>
              </li>
              <li>
                <span>Tax:</span>
                <span>{{taxPrice | formatPrice}}</span>
              </li>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      ...
      totalPrice() {
        return this.subPrice + this.shippingPrice + this.taxPrice - this.discountPrice
      }
    },
    methods: {
      payment() {
        ...
        this.$axios.post('/users/payment', {
          ...
          if (res.data.status === ERR_OK) {
            this.$router.push({
              path: '/orderSuccess',
              query: {
                orderId: res.data.results.orderId
              }
            })
          }
        })
      ...
      _getCartList() {
        this.$axios.get('/users/cart').then(res => {
          if (res.data.status === ERR_OK) {
            this.cartList = res.data.results.filter(item => {
